...
CREATE TABLE IF NOT EXISTS ContestProblemSubmission (
    ...
    CHECK (
        (SELECT COUNT(*)
         ...
         WHERE participant_id = NEW.participant_id AND problem_id = NEW.problem_id AND contest_id = NEW.contest_id) < 5
    ) -- Constraint for max submissions per problem per contest
);
